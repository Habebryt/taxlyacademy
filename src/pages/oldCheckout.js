...
const savePaidEnrollment = async (enrollmentData) => {
  ...
  try {
    ...
    console.error("Error saving paid enrollment:", error);
    throw error;
  }
};
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (code === "NGN") {
      config = {
        ...
        displayPrice: `₦${convertedAmount.toLocaleString()}`,
        gateway: "paystack",
      };
    } else {
      config = {
  ...
  const handlePaymentSuccess = () => {
    setModalContent({
      ...
      message: `Your enrollment for the ${selectedCourse.title} certificate is confirmed. Check your email for next steps.`,
    });
  };
  const handlePaymentError = () => {
    setModalContent({
      ...
      title: "Oh no!",
      message:
        "Your payment was successful, but we had trouble recording your enrollment. Please contact support.",
    });
  };
  ...
  if (isLoading) {
    return (
      <div className="container text-center py-5">
        ...
        <p className="mt-2 text-muted">Preparing secure payment options...</p>
      </div>
    );
  }
            ...
            <div className="col-lg-7" data-aos="fade-left">
              {authStatus === "pending" || !paymentConfig ? (
                ...
                <div className="alert alert-danger">
                  Could not connect to payment services. Please refresh and try
                  again.
                </div>
