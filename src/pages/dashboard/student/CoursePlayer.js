  ...
  return (
    <div className="mt-4">
      <ul className="nav nav-tabs">
        ...
        <li className="nav-item">
          <button
            className={`nav-link ${activeTab === "notepad" ? "active" : ""}`}
      ...
      <div className="card card-body border-top-0">
        {activeTab === "notes" && (
          <div>
            ...
            <p>
              This is where notes from the trainer for "
              {course.curriculum[activeLessonIndex]}" would appear. These could
              include key takeaways, links to resources, or downloadable
              materials.
            </p>
            ...
            {!isLessonCompleted && (
              <div className="mt-4 p-3 bg-light rounded border">
                ...
                <p className="small text-muted">
                  Ready to submit your work for this module? Click the button
                  below.
                </p>
                <Link
                  to={`/dashboard/submit-assignment/${courseId}/${activeLessonIndex}`}
...
const NotesPanel = ({ notes, setNotes, onSave, isSaving }) => (
  <div>
    ...
    <p className="small text-muted">
      Your notes are saved automatically and are only visible to you.
    </p>
    <textarea
...
const DiscussionPanel = ({ comments, onSubmit, isSubmitting }) => {
  ...
  return (
    <div>
      ...
      <div
        className="discussion-thread mb-3"
          ...
          comments.map((comment) => (
            <div key={comment.id} className="comment">
              ...
              <small className="text-muted">
                by {comment.authorName || "Student"} on{" "}
...
const CoursePlayer = () => {
  // --- FIX: The ID from the URL is the Firestore Document ID ---
  const { courseId: firestoreCourseId } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        // --- FIX: Use a direct document reference for efficiency ---
        // This is much faster and doesn't require a custom index.
        const courseDocRef = doc(db, "courses", firestoreCourseId);
        ...
        setCourse({ firestoreId: courseSnapshot.id, ...courseData });

        // The course's URL-friendly ID (e.g., "virtual-assistant-mastery") is stored in the document data
        const courseSlugId = courseData.id;
        const progressDocRef = doc(
          db,
          `artifacts/${appId}/users/${userId}/courseProgress`,
          courseSlugId
        );
    ...
    fetchData();
  }, [db, userId, firestoreCourseId, appId]);
  ...
  useEffect(() => {
    ...
    const fetchLessonData = async () => {
      const courseSlugId = course.id;
      const noteDocRef = doc(
        db,
        `artifacts/${appId}/users/${userId}/notes/${courseSlugId}/lesson`,
        `${activeLessonIndex}`
      );
      ...
      const commentsRef = collection(
        db,
        `courses/${course.firestoreId}/lessons/${activeLessonIndex}/comments`
      );
  ...
  const handleSaveNote = async () => {
    ...
    try {
      await setDoc(
        noteDocRef,
        { content: notes, updatedAt: serverTimestamp() },
        { merge: true }
      );
      alert("Note saved!");
    } catch (err) {
        ...
        <div className="row g-4">
          <div className="col-lg-8">
            ...
            <LessonContentTabs
              ...
              isSubmitting={isSubmitting}
              courseId={course.id} // Pass the slug ID
                  ...
                  return (
                    ...
                    >
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          <div className="fw-bold lesson-title">{`Module ${
                            index + 1
                          }`}</div>
