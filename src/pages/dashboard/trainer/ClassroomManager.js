...
const ClassroomManager = () => {
  ...
  const fetchClassroomData = useCallback(async () => {
    ...
    setError(null);
    try {
      // --- FIX: Query the collection to find the course by its URL-friendly ID ---
      ...
      const freeStudents = freeSnapshot.docs.map((doc) => ({
        ...doc.data(),
        enrollmentType: "Free",
      }));
      const certStudents = certSnapshot.docs.map((doc) => ({
        ...doc.data(),
      ...
      const studentProgressPromises = allStudents.map(async (student) => {
        const progressDocRef = doc(
          db,
          `artifacts/${appId}/users/${student.userId}/courseProgress`,
          courseId
        );
        ...
        return {
          ...student,
          progress: Math.round(
            (completedLessons.length / courseData.curriculum.length) * 100
          ),
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-5">
          ...
          <p className="mt-2 text-muted">Loading Classroom...</p>
        </div>
      </DashboardLayout>
    );
  }
        ...
        {students.length > 0 ? (
                    ...
                    {students.map((student, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <span
                            className={`badge ${
                              student.enrollmentType === "Certificate"
                        ...
                        <td>
                          ...
                          <small className="text-muted">
                            ...
                            {course.curriculum.length} lessons complete
                          </small>
                        </td>
                        <td>
                          <Link
                            to={`/dashboard/student-details/${courseId}/${student.userId}`}
          ...
          <div className="text-center py-5 border bg-light rounded">
            ...
            <p className="text-muted">
              Once students enroll, you can track their progress and manage your
              classroom here.
            </p>
          </div>
        )}
